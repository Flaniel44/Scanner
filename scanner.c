...
Token mlwpar_next_token(Buffer * sc_buf)
{
	...
	while (1){ /* endless loop broken by token returns it will generate a warning */
        short i = 0, getc=0; /*used for loops */
		...
		if (c == '!') { /*if !, possible comment */
			...
			if (b_setmark(sc_buf, b_getc_offset(sc_buf)) == NULL) /* set the mark to the beginning of the string */
				return error(B_SET_MARK_FAIL); /* runtime error*/
			if (b_setmark(str_LTBL, b_size(str_LTBL)) == NULL) /*set the mark of the string literal buffer to the spot where the next character will go*/
				return error(B_SET_MARK_FAIL); /* runtime error*/
			c = b_getc(sc_buf); /*get next char*/
			if (c == (unsigned char)R_FAIL_1 || c == (unsigned char)R_FAIL_2) /* runtime error*/
				return error(B_GETC_FAIL); 
			while (c != '"' || c == '\n') { /* check for string literal */
				...
				if (b_addc(str_LTBL, c) == NULL){ /* add character to string literal buffer */
					return error(B_ADDC_FAIL);} /* runtime error */
				c = b_getc(sc_buf); /*get next char*/
				if (c == (unsigned char)R_FAIL_1 || c == (unsigned char)R_FAIL_2) /* runtime error */
					return error(B_GETC_FAIL); 
				if (c == SEOF || c == 255) { /*if end of file reached*/
					t.code = ERR_T; /*set error token*/
					getc = b_getc_offset(sc_buf); /*capture the current getc offset to set the getc back once err_lex has been filled*/
					b_retract_to_mark(sc_buf);
					b_retract(sc_buf); /*retract one more to get quotation mark"*/
					if (b_size(str_LTBL) - b_mark(str_LTBL) > ERR_LEN) { /*if string to add is greater than 20 characters*/
						for (i = 0; i < ERR_LEN; i++) { /*for each char in the incorrect string literal */
							if (i < (ERR_LEN - 3)) { /*for the first 17 characters*/
								t.attribute.err_lex[i] = b_getc(sc_buf); /*add to err_lex*/
								if (t.attribute.err_lex[i] == 0) /* runtime error */
									return error(B_GETC_FAIL);
							} else /*... for the last 3 chars */
								t.attribute.err_lex[i] = '.';
						}
					} else { /*string literal is less than 20 characters, capture all*/
						for (i = 0; i < ERR_LEN; i++) { /*go through each char and add it to err_lex*/
							t.attribute.err_lex[i] = b_getc(sc_buf);
							if (t.attribute.err_lex[i] == SEOF) { break; } /*break when end of file char reached */
							if (t.attribute.err_lex[i] == 0) /* runtime error*/
								return error(B_GETC_FAIL);
						}
					}
					b_retract_to_mark(str_LTBL); /* use retract to mark to set the addc_offset of the string literal buffer*/
					if (b_size(str_LTBL) == R_FAIL_1) /*runtime error */
						return error(B_GET_MARK_FAIL);
					t.attribute.err_lex[i] = '\0'; /*close the err_lex string*/
					
					b_setmark(sc_buf, getc); /* set getc_offset back to beggining of string error*/
					b_retract_to_mark(sc_buf); 
					return t;
				}
			}/* end while */
			/* end of quotation marks*/
			if (b_addc(str_LTBL,'\0') == NULL) /* close string literal buffer */
				return error(B_ADDC_FAIL); /* runtime error*/
			t.code = STR_T; /* set string token */
			t.attribute.str_offset = b_mark(str_LTBL); /* set string offset */
			if (t.attribute.str_offset == R_FAIL_1) /*runtime error*/
				return error(B_GETC_FAIL); 
			return t;
		}
...
Token aa_func02(char lexeme[]){
	...
	if (strlen(lexeme) > VID_LEN) {
		for (i = 0; i < VID_LEN; i++)
			t.attribute.vid_lex[i] = lexeme[i]; /* only store first 8 characters in variable identifier */
	} else { /* variable identifier is shorter than 8 characters */
		for (i = 0; i < strlen(lexeme); i++)
			t.attribute.vid_lex[i] = lexeme[i]; /* store every character in variable identifier */
	}
	t.code = AVID_T; /* set Token code */
	t.attribute.vid_lex[i] = '\0'; /* append NULL terminator to variable identifier */
...
Token aa_func03(char lexeme[]){
	...
	if (strlen(lexeme) > VID_LEN) { /* maximum 8 characters in variable identifier */
		...
		t.attribute.vid_lex[VID_LEN] = '\0'; /* append NULL terminator to variable identifier */
	} else { /* variable identifier is shorter than 8 characters */
		for (i = 0; i < strlen(lexeme); i++)
			t.attribute.vid_lex[i] = lexeme[i]; /* store every character in variable identifier */
		t.attribute.vid_lex[i] = '\0'; /* append NULL terminator to variable identifier */
